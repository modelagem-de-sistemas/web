import { getJob, removeJob, updateJob } from '@/lib/job';
import { NextApiRequest, NextApiResponse } from 'next';

const get = async (req: NextApiRequest, res: NextApiResponse): Promise<void> => {
  const education = await getJob(Number(req.query.id));
...
const update = async (req: NextApiRequest, res: NextApiResponse): Promise<void> => {
  try {
    const data = await updateJob(Number(req.query.id), req.body);
...
const remove = async (req: NextApiRequest, res: NextApiResponse): Promise<void> => {
  try {
    const data = await removeJob(Number(req.query.id));
    res.status(200).json(data);
  } catch (e) {
    res.status(400).json({ message: e });
  }
